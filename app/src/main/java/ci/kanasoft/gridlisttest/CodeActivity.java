...
public class CodeActivity extends ActionBarActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//Log.i("Grid_List", "onCreate 1 : SourceActivity " + settings.getString(CompanyActivity.COMPANY_NAME, "valeur par défaut"));
		...
		GridView myGridView = (GridView) findViewById(R.id.gridCode);	
		
		UssdCode mUssdCode =  new UssdCode(getApplicationContext());
		ItemPullParser mParser = new ItemPullParser("code", R.raw.codeus01, this);		
		mSourceList = mUssdCode.fnSelectCodesFrom(mName, mCompanyName, mCategoryName);	
		...
		myGridView.setOnItemClickListener( new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				//nom de la source choisie: opération ou code
...
//				mIntent.putExtra(SourceActivity.SOURCE_NAME, mSource);					
				mIntent.putExtra(CODE_NAME, codeChoisie);
				
				kanaPreference.onSaveUserChoices( CODE_NAME, codeChoisie , getApplicationContext());
				
				Log.i("CodeActivity"," choix : " +codeChoisie) ;
